...
Our observations come from acoustic doppler current profiler (ACDP) buoys, which estimate the spectrum by measuring the wave induced velocity and surface height variations. According to \cite{vallis_atmospheric_2017}, the velocity and surface height anomaly induced by a simple wave with wave vector $\mathbf{k} = \left(k_x,k_y\right)$, frequency $\sigma$ and amplitude $a$ is
...
(Include ADCP locations if possible)
...
\begin{figure}
\centering
\includegraphics[width = .8\textwidth]{modelDomain}
...
where the observations $\y_t$ and matrices $\mathbf{H}_t,\mathbf{R}_t$ are valid at time $t$, and $\mathbf{\hat{h}}_t = \mathbf{H}_t\mathcal{M}_{t-1}\mathcal{M}_{t-2}\dots\mathcal{M}_0$.
...
where $\mathbf{\hat{H}}_t = \mathbf{H}_t\mathbf{M}_{t-1}\dots\mathbf{M_0}$ is the linearisation of $\mathbf{\hat{h}}_t$. Note that this cost function is further simplified since we have taken a background error covariance matrix of the form $\mathbf{B} = \mathbf{X}'_b\mathbf{X}_b^{\prime\top}$. This cost function has gradient
...
\[
\mathbf{\hat{H}}_t \mathbf{X}'_b \approx \_\_\_\_\_
\]

This eliminates the need to compute a tangent linear and adjoint model, which is a computationally expensive aspect of most variational data assimilation schemes \cite{Bannister2017}.