...
where the observations $\y_t$ and matrices $\mathbf{H}_t,\mathbf{R}_t$ are valid at time $t$, \comment{and}{$\hat{h}_{t}$ is an operator and should therefore be capatalized} $\mathbf{\hat{h}}_t = \mathbf{H}_t\mathcal{M}_{0 \rightarrow t}$ using the abbreviation $ \mathcal{M}_{0 \rightarrow t} = \mathcal{M}_{t-1}\mathcal{M}_{t-2}\dots\mathcal{M}_0$.
