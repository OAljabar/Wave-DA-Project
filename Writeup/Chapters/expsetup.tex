...
\end{subfigure}
\caption{Left: $H_s$ (colours) and direction (arrows) in the GoM at 18Z on 12/03/2022. Right: Surface analysis chart valid for 18Z on 12/03/2022, taken from NOAA WPC surface analysis archive \citep{NOAA2022}.}
...
Our observations come from an acoustic doppler current profiler (ACDP) buoy (cite), which estimates the spectrum by measuring the wave induced velocity and surface height variations. According to \citet{Vallis2017}, the velocity and surface height anomaly induced by a simple wave with wave vector $\mathbf{k} = \left(k_x,k_y\right)$, frequency $\sigma$ and amplitude $a$ is
