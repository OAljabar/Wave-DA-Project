...
From the forecasted spectra, other variables such as wave height, period and mean direction are derived. The three variables we will be focusing on in this study are significant wave height $H_s$, and two measures of the wave period $T_{m01}$ and $T_{m02}$. These are related to the spectrum via the spectral moments (equations \ref{eqn:moments}, \ref{eqn:mainvars}).
...
A limitation of the OSSE method is its inability to correct biases in the model. Figure \ref{fig:hsominusb} shows a possible bias in the forecasted values of $H_s$, which would be desirable to try to correct with DA. Since the data that we assimilate in this project is also from model output, it will be subject to all the same biases as the forecast data so when generating the observations as described in the previous paragraph it is not possible for the DA to correct these biases.

It is possible to add an artificial bias when generating the observations, but this is not pursued in this project in the interests of simplicity. (Theoretically, you could add a bias to the the truth and see whether the DA can correct it. For simplicity, we haven't looked into a bias correcting scheme. You might want to mention that bias correcting DA schemes exist (the MetOffice has one) and then add some references to papers using them.)
...
To get around this, at each analysis step we correct an entire 7-day forecast window, and in place of the forecast step we load data from the next forecast window. A schematic of the process is included in figure \ref{fig:ETKFschematic}. So, in our setup, each 7-day forecast window is treated as a stand-alone DA problem with its own truth. One ramification of this is that improvements in one window are not propagated to the next.
...
In this way, leadtime within a forecast window is treated the same as the two spatial coordinates by the DA system. Since there are 169 timesteps in each window, and we have three state variables on a $181 \times 210$ grid, the dimension of the state space is approximately $2 \times 10^7$.

At each analysis step, we will assimilate observations from the first three days of the forecast window. This allows us to investigate errors at times after the assimilation window and assess the capability of DA to improve the forecast at future time when observations are unavailable. This means assimilating observations of the three state variables ($H_s$, $T_{m01}$ and $T_{m02}$) from the first 73 timesteps of each window, so the dimension of the observations space is 219.

Since extremes of $H_s$ are the most dangerous wave events likely to be faced in offshore operations, $H_s$ is considered to be the most important variable out of the three assimilated. This gives motivation to perform two DA experiments: one in which which $H_s$ alone is assimilated, referred to as $H_s$ DA, and one in which the three variables are assimilated together, called all vars DA.
