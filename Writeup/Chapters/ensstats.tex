...
\includegraphics[width = \textwidth]{buoyhsvar}
\end{subfigure}
\begin{subfigure}{.49\textwidth}
\includegraphics[width = \textwidth]{meanhsvar}
\end{subfigure}
...
\centering
\begin{subfigure}{.49\textwidth}
\includegraphics[width = \textwidth]{buoyt01var}
\end{subfigure}
\begin{subfigure}{.49\textwidth}
\includegraphics[width = \textwidth]{meant01var}
\end{subfigure}
...
The background error covariance matrix is central to our DA system as it determines how infomation from the observations is propagated around the domain by DA. In this project it is approximated by the forecast error covariance matrix, which is derived from the ensemble perturbations $\x_t - \x_t^{(i)}$ as defined in equation \ref{eqn:ferrorcov}. The covariance values are illustrated here with correlation maps

\subsection{Spatial Covariances}

Figure (???) shows the corelation between Hs at the buoy location and Hs at all points in the domain. The correlation is calculated using the ensemble data from 12:00 on 06/03/2022
