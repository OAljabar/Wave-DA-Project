...
class DAS:
    ...
    def run(self, steps, verbose=ScreenOutput.standard):
        ...
        # flag for tracking if we are on first timestep
        first_step = True
        ...
        while step_da<=self.final_step:
            ...
            for model in self.models_p:
                ...
                if status_pdaf!=int(PdafError.none):
                    self.pe.comm_world.throw_error(f"Get_state critically failed to copy ensemble member at step {step_da}.")  
                ...
                if verbose==ScreenOutput.standard:
                    ...
                    print('Prestate min/max/mean at process {:d}: {:.3e} {:.3e} {:.3e}'.format(self.pe.comm_model.mype,
                                                                                np.nanmin(state_p),np.nanmax(state_p),
                                                                                np.nanmean(state_p)))
                model.step_forward(model.step, min(self.final_step - model.step - 1, steps_forward))
                if verbose==ScreenOutput.standard:
                    state_p = model.collect_state_pdaf(model.dim_state,None)
                    print('Poststate min/max/mean at process {:d}: {:.3e} {:.3e} {:.3e}'.format(self.pe.comm_model.mype,
                                                                                 np.nanmin(state_p),np.nanmax(state_p),
                                                                                 np.nanmean(state_p)))
                ...
                # if status_pdaf!=int(PdafError.none):
                #     self.pe.comm_world.throw_error(f"Put_state critically failed to copy ensemble member at step {step_da}.")  
