...
class PointObservation(Observation):
    ...
    def write_output(self,savedir = ''):
        ...
        values = self.read_obs.data['observed']
        coords = self.read_obs.data['coord']
        times = self.read_obs.data['time']
        nlt = int(max(coords[:,0])+1)
        nt = len(times)//nlt
        #print(nt,',',nlt)
        values = np.reshape(values,[nt,nlt],order = 'C')
        
        trutharray = xr.Dataset()
        trutharray['hs'] = xr.DataArray(values,coords = {'step':range(nt),'leadtime':np.arange(nlt)/24})
        
        filename = savedir + 'truth.nc'

        trutharray.astype('f8').to_netcdf(filename)
